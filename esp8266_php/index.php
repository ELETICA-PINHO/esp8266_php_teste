...
if (isset($_POST['toggle_LED'])) {
	...
	if($row['status'] == 0){
		...
		$update = mysqli_query($conn, "INSERT INTO `logs` (`status`, `exe`, `hora`,`data`) VALUES ('1','WEB-LIG', '$hora','$data')");
		
	
	}		
	else{
		$update = mysqli_query($conn, "UPDATE led_status SET status = 0 WHERE id = 1;");		
		$update = mysqli_query($conn, "INSERT INTO `logs` (`status`, `exe`, `hora`,`data`) VALUES ('0','WEB-DES', '$hora','$data')");
	}
}
